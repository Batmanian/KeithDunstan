...
topics:
  ...
  - name: Melway
    description: The Melway is the spiral-bound street directory that was, before satellite navigation, the essential map of Melbourne kept in every glovebox, a level of local knowledge Keith Dunstan joked his runaway poodle didn't need to find her own way home.
  - name: Menzies Hotel
...
Generated by `npm run generate-topics` from every `tags:` entry across the site's content files that has at least 2 page entries (topics below that only appear here if they already have a hand-written description). Re-running the script preserves existing `description` values and only appends topics it hasn't seen before — it never removes or overwrites one. Fill in `description` for each entry as time allows.
